...
public class HomeAPIFetcher extends AsyncTask<String, Integer, String> {
    private static final String DOMAIN_NAME = "http://192.168.128.41/tween";
    private HomeInterface context;

    HomeAPIFetcher(HomeInterface ctx){
        this.context = ctx;
    }
        ...
        try {
            ...
            if(response.has("result") && response.getString("result").equalsIgnoreCase("true")){
                if(response.has("tips")){
                    context.post_update_user_tips(response.getString("tips"));
                }
                else {
                    Log.d("Exe", "Your health tips has been updated!");
                }
            }
